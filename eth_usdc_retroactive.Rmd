...
eth_prices <- eth_prices[,c("BLOCK_NUMBER", "ETH_MARKET_PRICE")]
colnames(eth_prices) <- c("BLOCK_NUMBER","eth_price")
...
It is important to not overfocus on this small example as indicative of a liquidity provider's experience on Uniswap v3. In the real world, Bob's liquidity would be part of one of the largest pools in Uniswap; 5 ETH would have minimal price impact; and he would only have his balance of tokens change (and fees accumulated) proportional to the liquidity he provides. In addition, numerous trades in both directions would accumulate fees while negating price movements that increase divergent loss.
...
Some protocols (e.g. Gamma.xyz) collect individual deposits into Uni v3 positions within a Vault that acts as a custom NFT Position Manager. Thus, their positions won't use the Uniswap NFT Position Manager and will lack a Non-fungible Token ID (NF_TOKEN_ID).

Of the `r nrow(lp_actions)` rows in the weth-usdc liquidity pool actions history up to 
block height 15,576,600, there are `r sum(is.na(lp_actions$NF_TOKEN_ID))` entries without an 
NF Token ID.

`r mean(is.na(lp_actions %>% filter(NF_POSITION_MANAGER_ADDRESS != '0xc36442b4a4522e871399cd717abdd847ab11fe88') %>% select(NF_TOKEN_ID))) * 100`% of the non-Uniswap NFT Position Managers are missing NF Token IDs (as expected). 

`r mean(is.na(lp_actions %>% filter(NF_POSITION_MANAGER_ADDRESS == '0xc36442b4a4522e871399cd717abdd847ab11fe88') %>% select(NF_TOKEN_ID)))`% of the Uniswap NFT Position Manager actions are missing NF Token IDs (as expected).
...
- Otherwise, because the token ID will be NA, concatenate the NF_POSITION_MANAGER--TICK_LOWER--TICK_UPPER as a unique position.

Here is an example row of the LP Actions table transposed to break down how to read Liquidity Pool Actions.
...
the liquidity ACTION for the position was `DECREASE_LIQUIDITY`. 

- The position with its liquidity lowered was 
position `317705` and the AMOUNT0_ADJUSTED (USDC) was `26442.68` while the AMOUNT1_ADJUSTED was 
`14.6205`. 

- The LIQUIDITY is a large number `2.0122 * 10^16` and the tokens, again, are TOKEN0_SYMBOL `USDC` and TOKEN1_SYMBOL `WETH`.

- Position `317705` represents liquidity deposited in the range of TICK_LOWER `203740` and 
`205010`. 

Notice TICK_LOWER is a smaller number than TICK_UPPER. Some Uniswap v3 pools will have negative
values for these ticks; whether ticks are positive or negative depends on whether 1 unit of TOKEN0 or TOKEN1 is worth more. 

- PRICE_LOWER_0_1 and PRICE_UPPER_0_1 are the ticks converted into USDC/ETH price for easier readability; technically the contract uses TOKEN1/TOKEN0 for internal pricing.

- The LIQUIDITY_PROVIDER is the individual address handling the liquidity, but note, NFT positions are transferable so it is the NFT ID that is the best unique identifier of how positions change.

- The NF_POSITION_MANAGER_ADDRESS is most often the Uniswap v3 NFT Contract, but in cases here the NF_TOKEN_ID is missing, this will be a custom position manager contract (e.g., Gamma.xyz). So NF_POSITION_MANAGER_ADDRESS--TICK_LOWER--TICK_UPPER is a unique identifier for those positions.
...
is that as price moves, the more narrow your range, the faster you sell your proportion of the higher priced asset.
...
Given any particular price, it is possible to assess each positions allocation of assets if it is known how much liquidity they provided and the range they provided that liquidity. But it is not tracked and emitted by the contract every block.

This is partially why changes in his absolute value (e.g., his total USDC value changing from 220,000 to 218786.7117) are sometimes called *impermanent* loss (also known as Divergent Loss) - because trades in the other direction change this value and thus loss is not permanent until assets are withdrawn and the gain/loss is realized.
...
Thus, this analysis will focus on *realized* profit or loss. Someone puts liquidity with tokens in a range at Time [1]. Then later, they remove their liquidity and get tokens at Time [2]. Differences in the amount of tokens received (both from the price changes and from fees accumulated) and the market price determine whether the person has profited or lost over time.

### Identifying a Closed Position 

To identify a closed position is pretty simple: 

- For each unique position (NF Token ID or nf_position_manager_address-tick_lower-tick_upper)
...
Of the `r nrow(lp_actions)` where liquidity is added or removed there are `r length(unique(unique(lp_actions$unique_id)))` unique positions.
...
The following method is used to assess the reference price at any block height.

 - For every block where an ETH-stablecoin trade occurs in the ETH-USDC: 0.05%, 0.3%, 1%; ETH-DAI 0.3%; ETH-USDT 0.05%, 0.3% pools on Ethereum mainnet, the ETH volume weighted average swap price (VWAP) is identified while treating each stablecoin as exactly = 1 dollar.
- This VWAP is used get a single market price at the block level, where blocks without any trades are filled with the most recent block's VWAP. 
- A 99 block rolling median is then calculated to smooth spikes in price across blocks to better represent a broader market price.
...
# fee correction in key_functions.R
...
# accounting() from key_functions.R

a96877 <- accounting(id_lp_actions = p96877, id_fees = f96877, price_col = "eth_price")
...
# pnl from key_functions.R

pnl_96877 <- pnl(id_accounting = a96877, 
                 t0col = "usd_terms", t1col = "eth_terms",
                 price_col = "eth_price", price_base = "t0/t1")
```
...
#hodl_reference() available in key_functions.R

hodl96877 <- hodl_reference(id_accounting = a96877, 
                 t0col = "usd_terms", t1col = "eth_terms",
                 price_col = "eth_price", price_base = "t0/t1")
...
# strategy_reference() available in key_functions.R 

strat96877 <- strategy_reference(id_accounting = a96877, 
                   t0col = "usd_terms", t1col = "eth_terms",
                   price_col = "eth_price", price_base = "t0/t1")
...
Because non-Uniswap NFT Position Managers (e.g., Gamma.xyz) may or may not have transferable
ownership of individual assets within a grouped position, the LIQUIDITY_PROVIDER may or may not 
be consistent for a single NFT Position without an NF_TOKEN_ID (the ideal unique identifier).

Thus, the logical backup position identifier would be to focus on the Manager-position relationship 
as the direct intermediary between a person and Uniswap v3. 

NF_POSITION_MANAGER_ADDRESS--TICK_LOWER--TICK_UPPER is used as unique ID for grouped managed positions to understand PnL at the manager-position level. Which makes more sense as this analysis is not focused on 
individuals, but the positions individuals (including Vaults as manager of many people's positions) make.
...
Of the `r length(unique(closed_lp_actions$unique_id))` unique closed positions 
`r sum(! unique(closed_lp_actions$unique_id) %in% unique(closed_fees$unique_id))` did not collect their 
fees. So we will filter to the `r length(unique(closed_lp_w_fees$unique_id))` positions that can have their post-fees 
PnL and reference values calculated.

## Double Check the Fee collection quirk

By ensuring all fee collections are 1:1 at the tx_hash-unique_id level; we get a single fees 
row per fee collection and then remove the double counting once, i.e., for transactions where
fees are collected, then liquidity is removed, then fees are collected again (it happens rarely)
the collections are combined prior to correction. 

```{r}
max_tx_hash_unique_id_pairs_in_fees <- closed_fees %>% 
  group_by(TX_HASH, unique_id) %>% 
  summarise(n = n(), .groups = "drop") %>% 
  summarise(max = max(n))

```

We can confirm that the maximum fee rows for a tx_hash-unique_id pair is 
`r max_tx_hash_unique_id_pairs_in_fees$max`.

The next step is to remove any double counting of withdrawals within fees because 
the Uniswap v3 contract can bundle both as a Collect() action.
...
names(id_fees_fixed) <- names(id_fees_list)

# the following ID numbers had to be manually assessed because they removed liquidity
# in one transaction, but didn't withdraw that liquidity until a different transaction 
# where they withdrew it w/ fees; 
# the correction for subtraction cannot be safely done outside of the same tx hash
# so a few outliers in gains over HODL had to be manually assessed

 
simple_id_fixes = c('257807', '312929', '65786', '65812', '307159',
               '313558','308037', '308609', '309526','309951',
               '310633','56134','309447','309474','308843',
               '308019','299771','299775','307179',
               '308788', '309418','53369', '74271', '65807',
               '305503', '55835', '56064', '317893'
               )

for(i in simple_id_fixes){
  id_fees_fixed[[i]][, c("AMOUNT0_ADJUSTED","AMOUNT1_ADJUSTED")] <- 
    id_fees_fixed[[i]][ , c("AMOUNT0_ADJUSTED","AMOUNT1_ADJUSTED")] - 
  id_lp_list[[i]] %>% filter(ACTION == "DECREASE_LIQUIDITY") %>% select(AMOUNT0_ADJUSTED, AMOUNT1_ADJUSTED)
}


complex_id_fixes = c('154096', '299787', '155945','56988', '194524')

# manually checked transactions to identify row match to adjust 

id_fees_fixed[["154096"]][1 , c("AMOUNT0_ADJUSTED","AMOUNT1_ADJUSTED")] <- {
  id_fees_fixed[["154096"]][1 , c("AMOUNT0_ADJUSTED","AMOUNT1_ADJUSTED")] - 
    id_lp_list[["154096"]][2, c("AMOUNT0_ADJUSTED","AMOUNT1_ADJUSTED")]
}

id_fees_fixed[["299787"]][2 , c("AMOUNT0_ADJUSTED","AMOUNT1_ADJUSTED")] <- {
  id_fees_fixed[["299787"]][2 , c("AMOUNT0_ADJUSTED","AMOUNT1_ADJUSTED")] - 
    id_lp_list[["299787"]][2, c("AMOUNT0_ADJUSTED","AMOUNT1_ADJUSTED")]
}

id_fees_fixed[["155945"]][1 , c("AMOUNT0_ADJUSTED","AMOUNT1_ADJUSTED")] <- {
  id_fees_fixed[["155945"]][1 , c("AMOUNT0_ADJUSTED","AMOUNT1_ADJUSTED")] - 
    id_lp_list[["155945"]][2, c("AMOUNT0_ADJUSTED","AMOUNT1_ADJUSTED")]
}

# extra tricky as there is a remove w/o withdrawal; then another remove + then 1 big withdraw
id_fees_fixed[["56988"]][1 , c("AMOUNT0_ADJUSTED","AMOUNT1_ADJUSTED")] <- {
  id_fees_fixed[["56988"]][1 , c("AMOUNT0_ADJUSTED","AMOUNT1_ADJUSTED")] - 
    id_lp_list[["56988"]][2, c("AMOUNT0_ADJUSTED","AMOUNT1_ADJUSTED")]
}

  
id_fees_fixed[["194524"]][2, c("AMOUNT0_ADJUSTED","AMOUNT1_ADJUSTED")] <- {
  id_fees_fixed[["194524"]][2 , c("AMOUNT0_ADJUSTED","AMOUNT1_ADJUSTED")] - 
    id_lp_list[["194524"]][3, c("AMOUNT0_ADJUSTED","AMOUNT1_ADJUSTED")]
}
...
## Create the position specific accounting table
...
id_accounting <- lapply(1:length(id_fees_fixed), 
                        FUN = function(i){
                          ...
                          accounting(id_lp_actions = id_lp_list[[i]],
                                     ...
                                     price_col = "eth_price"), error = function(e){
...
id_pnl <- lapply(id_accounting, function(i){
  pnl(id_accounting = i ,t0col = "pnl_usdc_terms",t1col = "pnl_eth_terms",
      price_col = "eth_price",price_base = "t0/t1")
  })
...
id_hodl <- lapply(id_accounting, function(i){
  hodl_reference(id_accounting = i ,t0col = "hodl_usdc_terms",t1col = "hodl_eth_terms",
      price_col = "eth_price",price_base = "t0/t1")
})
hodl_results <- bind_rows(id_hodl, .id = "unique_id")

id_strat <- lapply(id_accounting, function(i){
  strategy_reference(id_accounting = i ,t0col = "strat_usdc_terms",t1col = "strat_eth_terms",
      price_col = "eth_price",price_base = "t0/t1")
})
