...
market_eth_price_at_block <- function(eth_prices, block){
 eth_prices$ETH_MARKET_PRICE[eth_prices$BLOCK_NUMBER %in% block]
}
...
accounting <- function(id_lp_actions, id_fees, eth_prices){
  ...
  id_lp_actions$eth_price <- market_eth_price_at_block(eth_prices, id_lp_actions$BLOCK_NUMBER)
...
pnl <- function(id_accounting){
 
pnl = data.frame(
  pnl_usd_terms = sum(id_accounting$token0, (id_accounting$token1*id_accounting$eth_price)),
  pnl_eth_terms = sum(id_accounting$token1, (id_accounting$token0/id_accounting$eth_price))
)
...
price_at_close_block = id_accounting$eth_price[id_accounting$BLOCK_NUMBER == max(id_accounting$BLOCK_NUMBER)][1] 
...
This section will be updated upon completion of some data adjustments. For now, 
for NFT Positions without an NF_TOKEN_ID, there is a misalignment of the unique identifier 
between LP_ACTIONS and the FEES tables. This is especially prevalent when users use a Uni v3 
Vault (e.g., Visor) to participate in managed group positions. The LP_ACTIONS table treats
Visor as the LIQUIDITY_PROVIDER for managed positions without an NF_TOKEN_ID, while FEES treats 
the individual withdrawing from a Visor position as the LIQUIDITY_PROVIDER for a managed positions.

Because we cannot identify transfers of Visor (or other vault) tokens, the LIQUIDITY_PROVIDER 
breaks the ability to identify a closed position with a unique ID when an NF_TOKEN_ID is not available (i.e., 
someone could collect fees for a position they didn't open and there is no unique ID to clarify when it was opened).

The incoming fix is to align LIQUIDITY_PROVIDER in both tables to the FEES table (individual withdrawing)
and ensure NF_POSITION_MANAGER is never `NULL` by moving the manager contract (e.g., Visor's ETH-USDC Vault)
as the NF_POSITION_MANAGER. 

This makes it possible to use NF_POSITION_MANAGER-TICK_LOWER-TICK_UPPER as a unique ID for grouped managed positions
and understand PnL at the manager (e.g., Visor) level. Which makes more sense as this analysis is not focused on 
individuals, but the positions individuals (including Visor as the manager of many people's positions) make.

For now, we will continue by filtering to only those positions where an NF_TOKEN_ID is available.

```{r}

closed_fees <- fees %>% 
  filter( !is.na(NF_TOKEN_ID) & unique_id %in% closed_positions$unique_id)
