...
accounting <- function(id_lp_actions, id_fees, eth_prices){
  ...
  # fastest way to conditionally subset potentially duplicated blocks for pricing 
  if(sum(duplicated(id_lp_actions$BLOCK_NUMBER)) == 0){
  id_lp_actions$eth_price <- market_eth_price_at_block(eth_prices, id_lp_actions$BLOCK_NUMBER)  
  } else {
    id_lp_actions$eth_price <- sapply(id_lp_actions$BLOCK_NUMBER, market_eth_price_at_block, eth_prices = eth_prices)
  }
